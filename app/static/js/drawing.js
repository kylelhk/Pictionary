...
function draw(event) {
  ...
  let x =
    ((event.pageX - canvasBounds.left - scrollX) / canvasBounds.width) *
    canvas.width;
  let y =
    ((event.pageY - canvasBounds.top - scrollY) / canvasBounds.height) *
    canvas.height;
  context.lineTo(x, y);
  context.stroke();
}
...
function updateBrushSize() {
  context.lineWidth = document.querySelector(
    'input[name="brush-size"]:checked'
  ).value;
}
...
function updateBrushColour() {
  context.strokeStyle = document.querySelector(
    'input[name="brush-colour"]:checked'
  ).value;
}
...
function saveDrawing() {
  ...
  const wordId = document
    .getElementById("word-to-draw")
